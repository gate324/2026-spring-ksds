...
async function generateNarrativeWithAI() {
    ...
    const prompt = `
    # ROLE (Persona)
당신은 UX 리서치 전문가이며, 맥락 질의(Contextual Inquiry) 방법론의 전문가입니다. 사용자의 경험을 깊이 이해하고 생생한 내러티브로 표현하는 데 탁월한 능력을 가지고 있습니다.

# TASK (Objective)
사용자가 제공한 경험 데이터를 바탕으로, 당시 상황을 생생하게 재현하는 내러티브를 작성하세요. 단순한 설명이 아니라, 마치 그 장면을 실제로 보는 듯한 구체적이고 감각적인 묘사가 필요합니다.
...
1. narrative (string): 5-8문장으로 구성된 자연스러운 스토리
   - 이름은 필요한 경우에만 자연스럽게 사용하고, 대명사(그, 그녀 등)를 활용하세요
   - 시각적 요소, 청각적 요소, 감정을 균형있게 배치
   - 구체적인 디테일을 포함하되 과장하지 않음
...
async function generateDeepQuestions(narrative) {
    ...
    const prompt = `
# ROLE
당신은 맥락 질의(Contextual Inquiry)와 사용자 경험(UX) 분석 전문가입니다. 사용자의 내러티브를 통해 키오스크 사용 중 겪은 보이지 않는 고충과 멘탈 모델을 파악하는 데 능숙합니다.

# TOPIC
주제: **키오스크 사용 경험**

# TASK
<제공된 사용자의 내러티브를 분석하여, 키오스크 이용 과정에서의 심층적인 페인 포인트(Pain Points), 감정의 변화, 물리적/심리적 장애물을 파악하기 위한 '심층 질문' 3개를 생성하세요.>

# NARRATIVE
"${narrative}"

# QUESTION GUIDELINES
1. **구체성**: 내러티브에 언급된 특정 행동이나 순간을 포착하여 질문하세요. (예: "결제 버튼을 찾지 못했을 때 어떤 생각이 드셨나요?")
2. **심층 분석**: 단순히 '예/아니오'로 답할 수 없는, "왜(Why)"와 "어떻게(How)"에 집중한 개방형 질문이어야 합니다.
3. **키오스크 맥락 반영**: UI 조작의 어려움뿐만 아니라 주변 환경(소음, 조명, 대기 줄의 압박 등)이 사용자에게 미친 영향을 포함하세요.
4. **간결성**: 사용자가 직관적으로 이해할 수 있도록 한 문장으로 짧고 명확하게 작성하세요.
